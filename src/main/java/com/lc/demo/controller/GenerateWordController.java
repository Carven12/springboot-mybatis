...
import com.lc.demo.constants.CommonConstants;
import com.lc.demo.constants.ExcelConstants;
import com.lc.demo.constants.FileConstants;
import com.lc.demo.utils.ExcelUtils;
import com.lc.demo.utils.FileUtils;
...
//                    cellValue = cellValue + ExcelConstants.EXCEL_CONTENT_ROOM;
        ...
        // 获取模板文件位置
        String templatePath = ResourceUtils.getFile("classpath:static/template/广源广汇花园起诉状.docx").getPath();

        dataList.stream().forEach(map -> {
            try {
                FileOutputStream fileOutputStream = FileUtils.generateDoc(templatePath, map,
                        map.get(ExcelConstants.EXCEL_TITLE_ROOM_NUMBER)
